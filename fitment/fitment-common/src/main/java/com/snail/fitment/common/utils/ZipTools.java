...
public class ZipTools {
	...
	public static void unzip(String zipFilePath, String unzipFilePath,
		...
		System.out.println("zipFilePath "+zipFilePath+",unzipFilePath "+unzipFilePath);
		...
		if (!unzipFileDir.exists() || !unzipFileDir.isDirectory()) {
			unzipFileDir.mkdirs();
			System.out.println("创建解压缩文件保存的路径  unzipFilePath "+unzipFilePath);
		}
		...
		while (entries.hasMoreElements()) {
			...
			if(entryName.equals(entry.getName())){
				...
				System.out.println("构建压缩包中一个文件解压后保存的文件全路径  entryFilePath "+entryFilePath);
				// 构建解压后保存的文件夹路径
				index = entryFilePath.lastIndexOf(File.separator);
				System.out.println("构建解压后保存的文件夹路径  index "+index);
				if (index != -1) {
