...
public class FitmentConfig extends Configure {
	...
	public static long getReloadPropertiesInterval() {
		return getProperty("reload.properties.interval", 60000);
	}
	...
	/**
	 * 是否测试服务器，如果测试服务器，可以多返回一些数据供测试
	 */
	public static final boolean isTestServer = getProperty("server.test", false);
	public static final String WEB_SERVER_SECRET = getProperty("web.server.secret", "123qwe!!ASD");
	public static final String WEB_SERVER_PHOTO_URL = getProperty("web.server.photo.url",
			"http://192.168.1.129:5000/account/photo?resourceid=");

	public static final boolean endProxy = getProperty("end.proxy", false);
	public static final String debugUrl = getProperty("debug.listener.url", "http://0.0.0.0:9999/proxy");
	public static final String listenerUrl = getProperty("front.listener.url", "");
	public static final String listenerUrls = getProperty("front.listener.urls", "");
	public static final String proxyUrl = getProperty("end.proxy.url", "");

	/*----------------------协议相关配置------------------------*/
	/**
	 * 当 defaultIdleTimeout内没有给客户端发送数据时，断开连接 （默认使用服务端发送数据作为超时判断依据,
	 * 以避免客户端持续发送无意义的数据导致服务器不断开连接的情况）, 单位为毫秒, 一般无需修改
	 */
	 ...
	 * token超时时间，默认3 天
	 */
	public static final int defaultTokenTimeout=getProperty("protocol.defaultTokenTimeout",3*24*3600000);

	/**
	 * 心跳超时时间，默认为6分钟
	 */
	public static final int heartTimeout() {
		return getProperty("protocol.heartTimeout", 360000);
	};

	/**
	 * 服务端单向心跳间隔，服务端发包，客户端不回包，验证服务端保持连接方法，0 表示不发
	 */
	public static final int serverHeartInterval = getProperty("protocol.serverHeartInterval", 0); // 默认为
																									// 0
																									// ，不发，与以前的程序保持一致

	public static final int BP_NORMAL_TASK_POOL_CORE_SIZE = getProperty("bp.normal.pool.core", 2);
	public static final int BP_NORMAL_TASK_POOL_MAX_SIZE = getProperty("bp.normal.pool.max",
			Runtime.getRuntime().availableProcessors() * 10);
	public static final int BP_NORMAL_TASK_QUEUE_SIZE = getProperty("bp.normal.queue",
			BP_NORMAL_TASK_POOL_MAX_SIZE * 500 / 20);

	public static final int BP_FIRST_TASK_POOL_CORE_SIZE = getProperty("bp.first.pool.core", 2);
	public static final int BP_FIRST_TASK_POOL_MAX_SIZE = getProperty("bp.first.pool.max",
			Runtime.getRuntime().availableProcessors() * 10);
	public static final int BP_FIRST_TASK_QUEUE_SIZE = getProperty("bp.first.queue",
			BP_FIRST_TASK_POOL_MAX_SIZE * 1000 / 100);

	public static final int BP_SEND_TASK_POOL_CORE_SIZE = getProperty("bp.send.pool.core", 2);
	public static final int BP_SEND_TASK_POOL_MAX_SIZE = getProperty("bp.send.pool.max",
			Runtime.getRuntime().availableProcessors() * 10);
	public static final int BP_SEND_TASK_QUEUE_SIZE = getProperty("bp.send.queue",
			BP_SEND_TASK_POOL_MAX_SIZE * 1000 / 100);

	public static final int BP_PORT = getProperty("bp.port", 62722);
	...
	// 资源文件下载地址头
	public static final String RESOURCE_DOWNLOAD_ADDRESS = getProperty("http.download.address",
			"http://uniform.wqapp.cn/");
	/**
	 * 文件系统类型
	 */
	...
	public static final String CDN_DOWNLOAD_PATH = getProperty("cdn.download.path",
			"http://cdn.lanxin.cn/d/up/blueprint/");

	// 资源大小限制（2G）
	public static final long getResourceMaxSize() {
		return getProperty("resource.max.size", 100 * 1024 * 1024);
	};

	// 块资源大小限制（合理为10M，目前设置为100M同资源大小，因为web未实现分块上传下载）
	public static final long getPartResourceMaxSize() {
		return getProperty("part.resource.max.size", 2147483647);
	};
	...
	public static final String kafka_serializer_class = getProperty("serializer.class",
			"kafka.serializer.StringEncoder");
	public static final String kafka_key_serializer_class = getProperty("key.serializer.class",
			"kafka.serializer.StringEncoder");
	public static final String kafka_partitioner_class = getProperty("partitioner.class",
	...
	public static final int compressThreshold = getProperty("protocol.compressThreshold", 512); // 数据包压缩的阈值（body
																								// 超过此大小则进行压缩，单位为
																								// byte）
	...
	public static int getNormalElementLimit() {
		return getProperty("log.normalElementLimit", 10);
	}
	
	public static int getNormalCharLimit() {
		return getProperty("log.normalCharLimit", 2000);
	}
