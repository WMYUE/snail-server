package com.snail.fitment.common.io;
...
public class ZipTools {
	...
	public static void unzip(String zipFilePath, String unzipFilePath,
			boolean includeZipFileName, String entryName) throws Exception {
		log.debug("ZinpTools zipFilePath "+zipFilePath+",unzipFilePath "+unzipFilePath);
		File zipFile = new File(zipFilePath);
		if (StringUtils.isEmpty(zipFilePath)
				|| StringUtils.isEmpty(unzipFilePath) || !zipFile.exists()) {
			throw new Exception();
		}
		
		// 如果解压后的文件保存路径包含压缩文件的文件名，则追加该文件名到解压路径
		if (includeZipFileName) {
			log.debug("zipFile.getName() "+zipFile.getName());
		...
		if (!unzipFileDir.exists() || !unzipFileDir.isDirectory()) {
			unzipFileDir.mkdirs();
			log.debug("创建解压缩文件保存的路径  unzipFilePath "+unzipFilePath);
		}
		...
		log.debug("ZipTools 循环对压缩包里的每一个文件进行解压  zipFile.getAbsolutePath() "+zipFile.getAbsolutePath());
		ZipFile zip = new ZipFile(zipFile);
		Enumeration<ZipEntry> entries = (Enumeration<ZipEntry>) zip.entries();
		log.debug("ZipTools 开始解压");
		// 循环对压缩包里的每一个文件进行解压
		while (entries.hasMoreElements()) {
			...
			if(entryName.equals(entry.getName())){
				...
				log.debug("构建压缩包中一个文件解压后保存的文件全路径  entryFilePath "+entryFilePath);
				// 构建解压后保存的文件夹路径
				index = entryFilePath.lastIndexOf("/");
				log.debug("构建解压后保存的文件夹路径  index "+index);
				if (index != -1) {
	 ...
	 *            解压后的文件保存的路径是否包含压缩文件的文件名。true-包含；false-不包含
	 * @param entryName 
	 * 			     包含entryName的文件才能被解压缩
	 */
	@SuppressWarnings("unchecked")
	public static Map<String,Object> regexUnzip(String zipFilePath, String unzipFilePath,
			boolean includeZipFileName, String entryName) throws Exception {
		log.debug("zipFilePath "+zipFilePath+",unzipFilePath "+unzipFilePath);
		if (StringUtils.isEmpty(zipFilePath)
		...
		while (entries.hasMoreElements()) {
			...
			if(entry.getName().contains(entryName)){
				...
				fileInfo.put("fileSize", entry.getSize());
				// 构建压缩包中一个文件解压后保存的文件全路径
				entryFilePath = unzipFilePath + File.separator + entry.getName();
				log.debug("构建压缩包中一个文件解压后保存的文件全路径  entryFilePath "+entryFilePath);
				// 构建解压后保存的文件夹路径
	...
	public static void main(String[] args) {
		...
		String zipFilePath = "D:\\1.zip";
		String unzipFilePath = "D:\\temp1";
		try {
			unzip(zipFilePath, unzipFilePath, true, "data.txt");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
