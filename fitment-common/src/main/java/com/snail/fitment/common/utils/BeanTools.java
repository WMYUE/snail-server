...
/**
 * 类似commons-beanutils的BeanUtils的工具类。
 * 实现了一些扩展策略。
 * 这个类不要有过多的依赖关系.<br>
...
public final class BeanTools {
    ...
    static {
    	transMap.put("name", "userName");
    	transMap.put("photoResId", "headerUrl");
    	transMap.put("orgName", "company");
    	transMap.put("roleCode", "roles");
    	transMap.put("userUniId", "userId");
    	transMap.put("userId", "userIdold");
    }
    private static final String getBpKey(String lxKey) {
    	String bpKey=transMap.get(lxKey);
    	if(bpKey == null) return lxKey;
    	return bpKey;
    }
    private BeanTools() {
        // utility class
    ...
    public static <T> Map<String, Object> turnToBlueprintUserMap(T bean) {  
        Map<String, Object> map = new HashMap<>();  
        if (bean != null) {  
            BeanMap beanMap = new BeanMap(bean);  
            for (Object key : beanMap.keySet()) {
            	map.put(getBpKey(key+""), beanMap.get(key));
            }
            String branch =  (String) beanMap.get("path");
            if(branch != null) {
	    		String department = branch.substring(branch.lastIndexOf("-") + 1);
	      		map.put(KEY_BRANCH_PATH+"", branch);
	      		map.put(KEY_DEPARTMENT+"", department);
            }
            String photoResId=(String) beanMap.get("photoResId");
            if(photoResId != null) {
            	map.put("photoFileName", FitmentConfig.WEB_SERVER_PHOTO_URL+ photoResId);
            }
        }  
        return map;  
    } 
    ...
    public interface PropertyInterceptor {
        ...
        public void afterGet(Object object, String name, Object key, Object value);
    }
    ...
    public static boolean isMappedProperty(Object bean, String name) 
            ...
            NoSuchMethodException {
        return PropertyUtils.getPropertyDescriptor(bean, name) 
                instanceof MappedPropertyDescriptor;
    }
    ...
    {
        PropertyUtils.NESTED_DELIM,
        PropertyUtils.MAPPED_DELIM,
        PropertyUtils.INDEXED_DELIM,
    };
    
    /**
     * 从PropertyUtils里面拷贝的setNestedProperty，做了如下改变
     * 1. 如果property名字路径中有null，如果value也是null，则直接返回，而不抛异常
     *   例如，name为propa.propb，propa取出是null，而value也是null，则直接返回
     * 2. 如果property名字路径中有null，而value不是null，则按照声明类型创建此对象
     * 3. 否则，如果value是null，而且目标字段是primitive类型，则直接返回
     * 4. 支持Indexed和Mapped语法直接嵌套的情况，甚至可以直接以[和(开头
     *    而不像BeanUtils中限制必须有名字
     * @param bean Bean object.
     * @param name Property name, maybe nestable (xxx.xxx).
     ...
     * @param autoCreateInterim 是否自动创建中间路径上的对象
     */
    public static void setProperty(Object bean, String name, Object value,
            boolean autoCreateInterim) 
            throws IllegalAccessException, InvocationTargetException, 
                    NoSuchMethodException, InstantiationException {
     ...
     * @param interceptor 对property的所有操作截取器
     */
    public static void setProperty(Object bean, String name, Object value,
        ...
        while (bean != null) {
            ...
            if (delimPos < 0) {
                // 最后一级，设置完后退出
                if (value != null || !getBeanPropertyClass(bean, name).isPrimitive()) {
                ...
                switch (delim) {
                    case PropertyUtils.NESTED_DELIM:
                        if (delimPos > 0) {
                            bean = getOrCreateSimpleProperty(bean, 
                                    name.substring(0, delimPos), 
                                    autoCreateInterim && value != null,
                                    interceptor);
                    ...
                    case PropertyUtils.MAPPED_DELIM:
                    {
                        ...
                        delimPos = name.indexOf(PropertyUtils.MAPPED_DELIM2, delimPos);
                    ...
                    {
                        int startPos = delimPos;
                        delimPos = name.indexOf(
                                PropertyUtils.INDEXED_DELIM2, delimPos);
                        String propName = name.substring(0, startPos);
                        // don't check exception
                        int index = Integer.parseInt(name.substring(
                                startPos + 1, delimPos));
                        if (delimPos == name.length() - 1) {
                            // 已经是最后一级了
     ...
     */
    public static Object getOrCreateSimpleProperty(Object bean, String propName,
            boolean create)
            throws IllegalAccessException, InvocationTargetException, 
                    NoSuchMethodException, InstantiationException {
    ...
    public static Object getOrCreateSimpleProperty(Object bean, String propName,
            boolean create, PropertyInterceptor interceptor)
            throws IllegalAccessException, InvocationTargetException, 
                    NoSuchMethodException, InstantiationException {
    ...
    /**
     * 支持Map的setSimpleProperty。
     * 如果value是null，而且目标字段是primitive类型，则直接返回
     */
     ...
     */
    public static void setMappedProperty(Object bean, String name1, String name2,
            Object value) 
            throws IllegalAccessException, InvocationTargetException,
                    NoSuchMethodException {
    ...
    /**
     * 从PropertyUtils里面拷贝的getNestedProperty，做了如下改变
     * 1. 如果property名字路径中有null，则返回null，而不抛异常
     *   例如，name为propa.propb，propa取出是null，则返回null
     * 2. 支持Indexed和Mapped语法直接嵌套的情况，甚至可以直接以[和(开头
     *    而不像BeanUtils中限制必须有名字
     * @param bean Bean object.
     * @param name Property name, maybe ("xxx.xxx")
     ...
     * @param interceptor property操作截取器 (允许null)
     */
    public static Object getProperty(Object bean, String name,
              PropertyInterceptor interceptor) 
              throws IllegalAccessException, InvocationTargetException,
        ...
        while (bean != null && name.length() > 0) {
            ...
            if (delimPos < 0) {
                ...
                switch (delim) {
                    ...
                    case PropertyUtils.INDEXED_DELIM: {
                        int startPos = delimPos;
                        delimPos = name.indexOf(PropertyUtils.INDEXED_DELIM2, delimPos);
                        bean = getIndexedProperty(bean, name.substring(0, startPos),
                                // don't check exception
                                Integer.parseInt(name.substring(startPos + 1, delimPos)),
                                interceptor);
                        break;
                    }
                    default:
    ...
    private static Object getDirectIndexed(Object array, int index, 
        ...
        if (interceptor != null) {
            interceptor.afterGet(array, null, Integer.valueOf((index)), result);
        }
        return result;
    }
    ...
    public static Object getIndexedProperty(Object bean, String name, int index,
        ...
        if (interceptor != null) {
            ...
            if (bean instanceof DynaBean || isIndexedProperty(bean, name)) {
                ...
                Object result = PropertyUtils.getIndexedProperty(bean, name, index);
                interceptor.afterGet(bean, name, Integer.valueOf(index), result);
                return result;
            }
            Object bean1 = getSimpleProperty(bean, name, interceptor);
    ...
    public static Object getMappedProperty(Object bean, String name1, 
        ...
        if (bean instanceof Map) {
            return getSimpleProperty(((Map)bean).get(name1), name2, interceptor);
        }
        if (interceptor != null) {
            ...
            if (bean instanceof DynaBean || isMappedProperty(bean, name1)) {
                ...
                Object result = PropertyUtils.getMappedProperty(bean, name1, name2);
