...
public class UrlUtils {
    ...
    public static URL parseURL(String address, Map<String, String> defaults) {
        ...
        if (address.indexOf("://") >= 0) {
            ...
            String[] addresses = RegisterConstants.COMMA_SPLIT_PATTERN.split(address);
            url = addresses[0];
            if (addresses.length > 1) {
                ...
                url += "?" + RegisterConstants.BACKUP_KEY + "=" + backup.toString();
            }
        }
    ...
    public static List<URL> parseURLs(String address, Map<String, String> defaults) {
        ...
        String[] addresses = RegisterConstants.REGISTRY_SPLIT_PATTERN.split(address);
    ...
    public static URL getEmptyUrl(String service, String category) {
        ...
        return URL.valueOf(RegisterConstants.EMPTY_PROTOCOL + "://0.0.0.0/" + service + "?" 
                    + RegisterConstants.CATEGORY_KEY + "=" + category
                    + (group == null ? "" : "&" + RegisterConstants.GROUP_KEY + "=" + group)
                    + (version == null ? "" : "&" + RegisterConstants.VERSION_KEY + "=" + version));
    }

    public static boolean isMatchCategory(String category, String categories) {
        if (categories == null || categories.length() == 0) {
            return RegisterConstants.DEFAULT_CATEGORY.equals(category);
        } else if (categories.contains(RegisterConstants.ANY_VALUE)) {
            return true;
        } else if (categories.contains(RegisterConstants.REMOVE_VALUE_PREFIX)) {
            return ! categories.contains(RegisterConstants.REMOVE_VALUE_PREFIX + category);
        } else {
    ...
    public static boolean isMatch(URL consumerUrl, URL providerUrl) {
        ...
        if( ! (RegisterConstants.ANY_VALUE.equals(consumerInterface) || StringUtils.isEquals(consumerInterface, providerInterface)) ) return false;
        
        if (! isMatchCategory(providerUrl.getParameter(RegisterConstants.CATEGORY_KEY, RegisterConstants.DEFAULT_CATEGORY), 
                consumerUrl.getParameter(RegisterConstants.CATEGORY_KEY, RegisterConstants.DEFAULT_CATEGORY))) {
            return false;
        }
        if (! providerUrl.getParameter(RegisterConstants.ENABLED_KEY, true) 
                && ! RegisterConstants.ANY_VALUE.equals(consumerUrl.getParameter(RegisterConstants.ENABLED_KEY))) {
            return false;
        }
       
        String consumerGroup = consumerUrl.getParameter(RegisterConstants.GROUP_KEY);
        String consumerVersion = consumerUrl.getParameter(RegisterConstants.VERSION_KEY);
        String consumerClassifier = consumerUrl.getParameter(RegisterConstants.CLASSIFIER_KEY, RegisterConstants.ANY_VALUE);
        
        String providerGroup = providerUrl.getParameter(RegisterConstants.GROUP_KEY);
        String providerVersion = providerUrl.getParameter(RegisterConstants.VERSION_KEY);
        String providerClassifier = providerUrl.getParameter(RegisterConstants.CLASSIFIER_KEY, RegisterConstants.ANY_VALUE);
        return (RegisterConstants.ANY_VALUE.equals(consumerGroup) || StringUtils.isEquals(consumerGroup, providerGroup) || StringUtils.isContains(consumerGroup, providerGroup))
               && (RegisterConstants.ANY_VALUE.equals(consumerVersion) || StringUtils.isEquals(consumerVersion, providerVersion))
               && (consumerClassifier == null || RegisterConstants.ANY_VALUE.equals(consumerClassifier) || StringUtils.isEquals(consumerClassifier, providerClassifier));
    }
    ...
    public static boolean isServiceKeyMatch(URL pattern, URL value) {
        return  pattern.getParameter(RegisterConstants.INTERFACE_KEY).equals(
            value.getParameter(RegisterConstants.INTERFACE_KEY))
            && isItemMatch(pattern.getParameter(RegisterConstants.GROUP_KEY),
                           value.getParameter(RegisterConstants.GROUP_KEY))
            && isItemMatch(pattern.getParameter(RegisterConstants.VERSION_KEY),
                           value.getParameter(RegisterConstants.VERSION_KEY));
    }
