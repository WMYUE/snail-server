...
public abstract class FailbackRegistry extends AbstractRegistry {
    ...
    // 定时任务执行器
    private final ScheduledExecutorService retryExecutor = Executors.newScheduledThreadPool(1, new BluePrintThreadFactory("DubboRegistryFailedRetryTimer"));
    ...
    public FailbackRegistry(URL url) {
        super(url);
        int retryPeriod = url.getParameter(RegisterConstants.REGISTRY_RETRY_PERIOD_KEY, RegisterConstants.DEFAULT_REGISTRY_RETRY_PERIOD);
    ...
    public void register(URL url) {
        ...
        try {
            ...
            // 如果开启了启动时检测，则直接抛出异常
            boolean check = getUrl().getParameter(RegisterConstants.CHECK_KEY, true)
                    && url.getParameter(RegisterConstants.CHECK_KEY, true)
                    && ! RegisterConstants.CONSUMER_PROTOCOL.equals(url.getProtocol());
    ...
    public void subscribe(URL url, NotifyListener listener) {
        ...
        try {
            ...
            if (urls != null && urls.size() > 0) {
                ...
                logger.error("Failed to subscribe " + url + ", Using cached list: " + urls + " from cache file: " + getUrl().getParameter(RegisterConstants.FILE_KEY, System.getProperty("user.home") + "/dubbo-registry-" + url.getHost() + ".cache") + ", cause: " + t.getMessage(), t);
            } else {
                // 如果开启了启动时检测，则直接抛出异常
                boolean check = getUrl().getParameter(RegisterConstants.CHECK_KEY, true)
                        && url.getParameter(RegisterConstants.CHECK_KEY, true);
